...
const formatDate = (value: string, format: DateTimeFormatOptions): string => {
  ...
  return timestamp || Date.parse(value)
    ? DateTime.fromISO(new Date(timestamp ? +value * 1000 : value).toISOString()).toLocaleString(format)
    : value;
};
...
export const DateFormats = Object.fromEntries(
  Object.entries(FormatOptions).map(([key, option]) => [key, formatDate(now, option)]),
);
...
export const Formatters: Record<string, Record<string, (value: string) => string>> = {
  [Type.DATE]: Object.fromEntries(
    Object.entries(FormatOptions).map(([key, option]) => [key, (value) => formatDate(value, option)]),
  ),
  [Type.NUMBER]: {
    ...
    [NumberFormatOptions.FLOAT_1_DEC]: (value) => {
      return _.isNumber(value) ? (Math.round(value * 10) / 10).toFixed(1) : value;
    },
    [NumberFormatOptions.FLOAT_2_DEC]: (value) => {
      return _.isNumber(value) ? (Math.round(value * 100) / 100).toFixed(2) : value;
    },
  },
  [Type.BOOLEAN]: {
